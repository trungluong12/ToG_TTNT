...
def construct_relation_prune_prompt(question, entity_name, total_relations, args):
    return extract_relation_prompt_wiki % (args.width, args.width)+question+'\nTopic Entity: '+entity_name+ '\n<Relations List>\n'+'\n'.join([f"{i}. wiki.relation.{item.replace(' ','_')} </Relations List>" for i, item in enumerate(total_relations, start=1)])+ " \nImportant Note: Only return relations that appear in the provided Relations List. Do not include any relations that are not listed. Keep the relations exactly as they appear in the list without modification.\n" + '\nA:'
...
def relation_search_prune_API(entity_id, entity_name, pre_relations, pre_head, question, args, wiki_client):
    
    total_relations, entity_names, entity_ids, id_to_label, label_to_id  = wikidata.run_item_id(entity_id)
...
def entity_search_API(entity, relation, wiki_client, head):
...
def entity_prune(total_entities_id, total_relations, total_candidates, total_topic_entities, total_head, total_scores, args, wiki_client, id_to_label):
    ...
    tops = [id_to_label[entity_id] if (entity_name := id_to_label[entity_id]) != "Not Found!" else "Unname_Entity" for entity_id in tops]
